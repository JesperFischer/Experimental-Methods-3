...
author: "Group 9: Jesper Fischer Ehmsen"
date: "03.09.2020"
...
```{r}
df1 = df %>% group_by(SUBJ) %>% summarize(mean((CHI_MLU))) %>% filter(df1$`mean(CHI_MLU)` > 2.7)

df2 = df %>% filter(CHI_MLU < 1.5 & VISIT.x == "1")
...
```{r}
df3 = df %>% filter(VISIT.x == "6")
df3$types_CHI = sort(df3$types_CHI, decreasing = TRUE)
...
```{r}
df4 = df %>% filter(Diagnosis == "ASD") %>% select(c("CHI_MLU","tokens_CHI", "Diagnosis", "CHI_MLU","Diagnosis"))
```
